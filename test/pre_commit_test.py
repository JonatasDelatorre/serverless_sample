...
def moto_s3():
    with moto.mock_s3():
        ...
        bucket.upload_file('datalake_sample/test/samples/source_csv.csv', 'source/source_csv.csv')
        bucket.upload_file('datalake_sample/test/samples/raw_parquet.parquet', 'raw/raw_parquet.parquet')
        yield bucket
...
class TestExtractFile(unittest.TestCase):    
    ...
    def test_lambda_sqs_message_input(self):
        ...
        with open('datalake_sample/test/samples/source_sqs_message.txt') as f:
            contents = f.read()
...
class TestProcessFile(unittest.TestCase):    
    ...
    def test_lambda_stepfunction_message_input(self):
        ...
        with open('datalake_sample/test/samples/raw_stepfunctions_message.txt') as f:
            contents = f.read()
            event = ast.literal_eval(contents)
